...
import com.michaelflisar.settings.demo.PrefUserSettings
...
import com.michaelflisar.settings.demo.databinding.ActivityDemo2Binding
...
class Demo2Activity : AppCompatActivity() {
    ...
    val onChangeCallback = object : ISettingsChangedCallback {
        override fun onSettingChanged(setting: ISetting<*>, customItem: SettingsCustomObject, oldValue: Any?, newValue: Any?) {
            ...
            val parent = when (customItem) {
                SettingsCustomObject.None -> "Global Settings"
                is SettingsCustomObject.Element -> (customItem.item as PrefUserSettings).userName
            }
            Toast.makeText(this@Demo2Activity, "[$parent] Setting '${setting.label.get()}' changed: $oldValue => $newValue", Toast.LENGTH_SHORT).show()
        ...
        override fun onCustomEnabledChanged(setting: ISetting<*>, customItem: SettingsCustomObject.Element, oldValue: Boolean, newValue: Boolean) {
            val parent = (customItem.item as PrefUserSettings).userName
            Toast.makeText(this@Demo2Activity, "[$parent] Setting '${setting.label.get()}' enable changed: $oldValue => $newValue", Toast.LENGTH_SHORT).show()
        }
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        setTheme(if (SettingsDefs.appSettingDarkTheme.readGlobal()) R.style.AppThemeDark else R.style.AppTheme)
    ...
    private fun updateUI() {
        ...
        val setup = SettingsDisplaySetup(
                showID = false,
                useDarkTheme = SettingsDefs.appSettingDarkTheme.readGlobal(),
...
//                        , groupTextColor = SettingsColor.Color(Color.WHITE)
        ...
        settings = Settings(
                ...
                if (selectedPrefItem == null) SettingsCustomObject.None else SettingsCustomObject.Element(selectedPrefItem!!),
                setup
        )

        val useList = SettingsDefs.appSettingLayoutStyle.readGlobal().id == AppLayoutStyleEnum.List.id
